...
class VideoJsonDataset(Dataset):
    ...
    def build_json(self, json_data):
        ...
        for data in json_data["data"]:
            for nested_data in data["data"]:
                self.build_json_dict(data, nested_data, extended_data)
    ...
    def get_frame_range(self, vr):
        return get_video_frames(
            vr, self.sample_start_idx, self.frame_step, self.n_sample_frames
        )
    ...
    def train_data_batch(self, index):
        # TODO: Add the first and initial frame loading for conditioning

        raise Exception("TODO: Add dataloading")
...
class SingleVideoDataset(Dataset):
    ...
    def single_video_batch(self, index):
        ...
        if train_data.endswith(self.vid_types):
            ...
            raise ValueError(
                f"Single video is not a video type. Types: {self.vid_types}"
            )
...
class VideoFolderDataset(Dataset):
    ...
    def get_frame_batch(self, vr, resize=None):
        ...
        idxs = every_nth_frame * np.arange(
            effective_idx, effective_idx + n_sample_frames
        )
