...
def save_pipe(
    ...
    unet_out = copy.deepcopy(
        accelerator.unwrap_model(unet.cpu(), keep_fp32_wrapper=False)
    )
    text_encoder_out = copy.deepcopy(
        accelerator.unwrap_model(text_encoder.cpu(), keep_fp32_wrapper=False)
    )
    ...
    lora_manager.save_lora_weights(
        model=pipeline, save_path=save_path, step=global_step
    )
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    text_encoder_lora_params, text_encoder_negation = lora_manager.add_lora_to_model(
        use_text_lora,
        text_encoder,
        lora_manager.text_encoder_replace_modules,
        lora_text_dropout,
        lora_path,
        r=lora_rank,
    )
    ...
    extra_text_encoder_params = (
        extra_unet_params if extra_unet_params is not None else {}
    )

    optim_params = [
        param_optim(
            unet,
            trainable_modules is not None,
    ...
    unet_and_text_g_c(
        unet, text_encoder, gradient_checkpointing, text_encoder_gradient_checkpointing
    )
    ...
    def finetune_unet(batch, train_encoder=False):
        ...
        if global_step == 0:
            ...
            handle_trainable_modules(
                unet, trainable_modules, is_enabled=True, negation=unet_negation
            )

            # unfreeze frame conditioner
            frame_conditioner.train()
        ...
        processed_images = image_processor(initial_images)
        initial_image_embeds = image_encoder(processed_images)

        # Process images
        final_images = batch["final_images"]
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                with accelerator.autocast():
                    loss, latents = finetune_unet(
                        batch, train_encoder=train_text_encoder
                    )
                ...
                if should_sample(global_step, validation_steps, validation_data):
                    ...
                    if accelerator.is_main_process:
                        ...
                        with accelerator.autocast():
                            ...
                            lora_manager.deactivate_lora_train(
                                [unet, text_encoder], True
                            )
                            ...
                            save_filename = (
                                f"{global_step}_dataset-{curr_dataset_name}_{prompt}"
                            )
